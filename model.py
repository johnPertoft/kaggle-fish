...
def conv2d(prev, name, shape, stride=(1, 1, 1, 1)):
    with tf.variable_scope(name) as scope:
        kernel = tf.Variable(tf.truncated_normal(shape, dtype=tf.float32, stddev=1e-1), name="weights")

        conv = tf.nn.conv2d(prev, kernel, stride, padding="SAME")

        biases = tf.Variable(tf.constant(0.0, shape=[shape[-1]], dtype=tf.float32), name="biases")
...
def dense(prev, name, output_dim, activation=True):
    with tf.variable_scope(name) as scope:
        ...
        weights = tf.Variable(tf.truncated_normal((shape, output_dim), dtype=tf.float32, stddev=1e-1), name="weights")

        biases = tf.Variable(tf.constant(1.0, shape=[output_dim], dtype=tf.float32), name="biases")
...
class Fishmodel:
    def __init__(self, X, num_classes):
        # TODO: just some random model to start with atm, probably need
        # something more complex for this task
        ...
        self.mpool1 = max_pool2d(self.conv2, "max_pool1", shape=(1, 2, 2, 1), stride=(1, 2, 2, 1))

        self.conv3 = conv2d(self.mpool1, "conv3", shape=(3, 3, 32, 64))
        self.conv4 = conv2d(self.conv3, "conv4", shape=(3, 3, 64, 64))
        self.mpool2 = max_pool2d(self.conv4, "max_pool2", shape=(1, 2, 2, 1), stride=(1, 2, 2, 1))
